...
import com.personal.spring_security.backend.services.JwtService;
    ...
    public void onAuthenticationSuccess(HttpServletRequest request,
        ...
        User user = userRepository.findByEmail(email)
                .map(existingUser -> {
                    ...
                    String currentUrl = existingUser.getPictureUrl();
                    if (currentUrl == null || currentUrl.startsWith("http")) {
                        log.info("Atualizando foto com a URL do Google para: {}", email);
                        existingUser.setPictureUrl(picture);
                    } else {
                        log.info("Mantendo foto local personalizada para o usuário: {}", email);
                    }
